...
class WP_Coach_JSON_Courses_Controller extends WP_JSON_Controller {
  ...
  public function register_routes() {
    ...
    register_json_route( 'plugin/wp-coach/' . $this->_version, '/courses', array(
      array(
    ...
    register_json_route( 'plugin/wp-coach/' . $this->_version, '/courses/(?P<id>[\d]+)', array(
      array(
        'methods'         => WP_JSON_Server::READABLE,
    ...
    register_json_route( 'plugin/wp-coach/' . $this->_version, '/courses/(?P<id>\d+)/revisions', array(
      'methods'         => WP_JSON_Server::READABLE,
    ...
    register_json_route( 'plugin/wp-coach/' . $this->_version, '/courses/schema', array(
      'methods'         => WP_JSON_Server::READABLE,
  ...
  public function get_items( $request ) {
    ...
    $args['post_type'] = 'wp_coach_course';
  ...
  public function get_item( $request ) {
    ...
    if ( empty( $id ) || empty( $post->ID ) || 'wp_coach_course' !== $post->post_type ) {
  ...
  public function create_item( $request ) {
    ...
    $post->post_type = 'wp_coach_course';
    ...
    $response->header( 'Location', json_url( '/plugin/wp-coach/' . $this->_version . '/courses/' . $post_id ) );
  ...
  protected function prepare_items_query( $prepared_args = array() ) {
    ...
    if ( empty( $query_args['post_status'] ) && 'attachment' === 'wp_coach_course' ) {
  ...
  protected function prepare_item_for_database( $request ) {
    ...
    if ( empty( $request['id'] ) ) {
      ...
      $prepared_post->post_type = 'wp_coach_course';
    } else {
    ...
    // Parent
    $post_type_obj = get_post_type_object( 'wp_coach_course' );
  ...
  protected function prepare_links( $post ) {
    $base = '/plugin/wp-coach/' . $this->_version . '/courses';
  ...
  public function get_item_schema() {
    ...
    foreach ( $post_type_attributes as $attribute ) {
      if ( isset( $fixed_schemas[ 'wp_coach_course' ] ) && ! in_array( $attribute, $fixed_schemas[ 'wp_coach_course' ] ) ) {
        continue;
      } elseif ( ! in_array( 'wp_coach_course', array_keys( $fixed_schemas ) ) && ! post_type_supports( 'wp_coach_course', $attribute ) ) {
        continue;
      }
